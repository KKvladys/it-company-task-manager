...
class LoginFormTests(TestCase):
    def test_valid_data(self):
        form = LoginForm(data={"username": "testuser", "password": "strongpassword"})
        self.assertTrue(form.is_valid())

    def test_missing_username(self):
        form = LoginForm(data={"password": "strongpassword"})
        self.assertFalse(form.is_valid())
...
class WorkerUpdateFormTests(TestCase):
    ...
    def test_invalid_position(self):
        form = WorkerUpdateForm(
            data={"first_name": "Updated", "last_name": "Name", "position": 999},
            instance=self.worker,
        )
        self.assertFalse(form.is_valid())
